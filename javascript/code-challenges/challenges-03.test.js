...
const notInFirstArray = (forbiddenValues, arr) => {
  const notInFirstArrFilteredArr = arr.filter(
...
const getBaseStatGreaterThan = (arr, minBaseStat) => {
  const filteredObjArr = arr.filter((item) => item.baseStat > minBaseStat);
...
const getStatName = (arr, minBaseStat) => {
  const onlyStatName = getBaseStatGreaterThan(arr, minBaseStat).map(
    (item) => item.stat.name
  );
  return onlyStatName;
};
...
const getCharactersWithoutChildren = (arr) => {
  const noChildrenArr = arr.filter((item) => {
...
const evenOddNumericValues = (arr) => {
  const removeNonNumeric = arr.filter((item) => !isNaN(item));
  const evenAndOdd = removeNonNumeric.map((num) => {
    if (num % 2 === 0) {
      return 'even';
    } else {
      return 'odd';
