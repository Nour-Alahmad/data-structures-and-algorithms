...
const addPurchases = (arr) => {
  let PurchaseSum = arr.reduce((accumulator, currValue) => {
    return accumulator + currValue.purchasePrice;
  }, 0);
  return PurchaseSum;
};
...
const countNumberOfChildren = (arr) => {
  const childrenArray = arr.map((item) => {
  ...
  for (let i = 0; i < childrenArray.length; i++) {
    if (childrenArray[i] !== undefined) {
      arrayToUse.push(childrenArray[i]);
    }
  }
  return arrayToUse
    .join()
...
const countPrimeNumbers = (arr) => {
  let index = 0;
  const primeNums = arr.reduce((accumulator, currentValue) => {
    if (isPrime(currentValue) && currentValue !== 1) {
      index++;
      return index;
    }
...
const extractStat = (statName, arr) => {
  let returnName = "";
  const matchName = arr.reduce((previousValue, currentValue) => {
    if (statName === currentValue.stat.name) {
      returnName = currentValue.stat;
      return returnName;
    }
    return returnName, previousValue;
  });
  return matchName;
};
...
const extractChildren = (arr) => {
  const namesWithLetterA = arr.filter((word) => word.name.includes("a"));

  const extractedChildrenArray = [];
  namesWithLetterA.reduce((accumulator, currentValue) => {
    extractedChildrenArray.push(currentValue.children);
    return extractedChildrenArray, accumulator;
  });

  return extractedChildrenArray;
};
